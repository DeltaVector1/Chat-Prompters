...
# Constants for the DanMistralV7Tekken format
format_tokens = {
    "starting_sequence": {
        "text": "<s>",
        "tokens": None
...
def tokenize_item(item, tokenizer, max_length):
    ...
    for turn_idx, turn in enumerate(turns):
        ...
        if from_value in system_from_values:
            turn_tokenized["input_ids"].extend(format_tokens["turn_suffix_system"]["tokens"])
            if should_contribute_to_loss:
                turn_tokenized["labels"].extend(format_tokens["turn_suffix_system"]["tokens"])
            else:
                turn_tokenized["labels"].extend([mask_token_id] * len(format_tokens["turn_suffix_system"]["tokens"]))
        elif from_value in user_from_values:
            turn_tokenized["input_ids"].extend(format_tokens["turn_suffix_user"]["tokens"])
            if should_contribute_to_loss:
                turn_tokenized["labels"].extend(format_tokens["turn_suffix_user"]["tokens"])
            else:
                turn_tokenized["labels"].extend([mask_token_id] * len(format_tokens["turn_suffix_user"]["tokens"]))
        elif from_value in assistant_from_values:
            turn_tokenized["input_ids"].extend(format_tokens["turn_suffix_assistant"]["tokens"])
            if should_contribute_to_loss:
                turn_tokenized["labels"].extend(format_tokens["turn_suffix_assistant"]["tokens"])
            else:
                turn_tokenized["labels"].extend([mask_token_id] * len(format_tokens["turn_suffix_assistant"]["tokens"]))
        elif from_value in tool_from_values:
            turn_tokenized["input_ids"].extend(format_tokens["turn_suffix_tool"]["tokens"])
            if should_contribute_to_loss:
                turn_tokenized["labels"].extend(format_tokens["turn_suffix_tool"]["tokens"])
            else:
                turn_tokenized["labels"].extend([mask_token_id] * len(format_tokens["turn_suffix_tool"]["tokens"]))
        else:
