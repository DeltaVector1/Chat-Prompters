...
def fill_format_tokens(tokenizer):
    ...
    Also handles the BOS token properly.
    """
    # First, handle the BOS token for starting_sequence
    if format_tokens["starting_sequence"]["tokens"] is None:
        if tokenizer.bos_token_id is not None:
            # Set the starting sequence to just the BOS token
            ...
            # If there's no BOS token, use an empty list
    ...
    for key, value in format_tokens.items():
        if key not in ["turn_seperator", "starting_sequence"] and value["tokens"] is None:
            ...
            # Use add_special_tokens=False to avoid accidentally adding BOS/EOS tokens
